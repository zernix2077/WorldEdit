...
/**
 * Expands the current selection along a direction, optionally on both sides.
 */
public final class ExpandCommand extends WorldEditCommand {
    public ExpandCommand() {
        super("expand", "worldedit:command.expand.description");
    }
        ...
        tree.getRoot()
                ...
                .exec((context, player) -> {
                    ...
                    if (Selection.applyBounds(player, out)) {
                        player.sendTranslatable(SelectionMessages.EXPANDED, out.volume() - bounds.volume());
                    }
                    return context.success();
